...
pub fn merge<C: Cache>(
    ...
    match (t1.into_owned(), t2.into_owned()) {
        ...
        (Term::Record(r1), Term::Record(r2)) => {
            ...
            m.extend(
                left.into_iter()
                    .map(|(id, field)| (id, field.map_value(|value| revert_closurize(value, &mut env, &env1)))),
            );
            m.extend(
                right
                    .into_iter()
                    .map(|(id, field)| (id, field.map_value(|value| revert_closurize(value, &mut env, &env2)))),
            );
            ...
            for (id, (field1, field2)) in center.into_iter() {
                m.insert(
                    ...
                    Field {
                        metadata: todo!(),
                        value: Some(fields_merge_closurize(&mut env, todo!(), &env1, todo!(), &env2, field_names.iter())?),
                    },
                );
            }
